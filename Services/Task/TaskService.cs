...
namespace TaskManager_Client.Services.Task
{
    public class TaskService:ITaskService
    {
        ...
        {
            RequestHelper.Client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenWraper.Token);
            var response = await RequestHelper.Client.GetAsync("api/Task/AvailableTasks");
            return response;
        }

        public async Task<HttpResponseMessage> SetTaskToUserAsync(int taskId)
        {
            var serializedUser = JsonConvert.SerializeObject(taskId);
            var stringContent = new StringContent(serializedUser, Encoding.UTF8, "application/json");
            var response = await RequestHelper.Client.PostAsync("api/Task/SetTaskToUser", stringContent);
            return response;
        }

        public async Task<HttpResponseMessage> TaskTypesAsync()
        {
            var response = await RequestHelper.Client.GetAsync("api/Task/TaskProgress");
            return response;
        }
    }
}
